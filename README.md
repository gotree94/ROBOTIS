# 🤖 ROBOTIS TurtleBot3

🔗 [TurtleBot3 e-Manual (공식 매뉴얼)](https://emanual.robotis.com/docs/en/platform/turtlebot3/overview/)

<img width="400" height="300" alt="025" src="https://github.com/user-attachments/assets/ff0ddfc3-ef9c-4ff9-b3f8-1e421bcec6c0" />

---

# 📘 RM 목차

1. [OpenCR1.0](#1-opencr10)  
   1.1 [CUBEIDE](#11-cubeide)  
   1.2 [Arduino](#12-arduino)  

2. [Raspberry PI 4](#2-raspberry-pi-4)  
   2.1 [리눅스 기본 명령어](#21-리눅스-기본-명령어)  
   2.2 [파이썬 프로그래밍](#22-파이썬-프로그래밍)  
   2.3 [GPIO 제어 : 센서, 모터, 통신](#23-gpio-제어--센서-모터-통신)  
   2.4 [네트워크 및 웹서버 구축, 카메라 제어, Pan/Tilt](#24-네트워크-및-웹서버-구축-카메라-제어-pantiltt)  

---

## 1. OpenCR1.0 (STM32F746ZGT6 / 32-bit ARM Cortex®-M7 with FPU (216MHz, 462DMIPS))

🔗 [OpenCR1.0 e-Manual](https://emanual.robotis.com/docs/en/parts/controller/opencr10/)  

<img width="320" height="240" alt="opencr_product" src="https://github.com/user-attachments/assets/dbb056d6-b38c-4c2f-bc4f-d92e221fe426" />

🔗 [ST-LINK 구매 링크 (미소파츠)](https://smartstore.naver.com/misoparts/products/9353479057) — 💰 예상가격 약 4,690원  

<img width="320" height="240" alt="027" src="https://github.com/user-attachments/assets/b4289b25-ddc1-41f3-b079-2ded316810e9" />

### 1.1 CUBEIDE
- 하드웨어 관련 리뷰  
- 소프트웨어 관련 리뷰  
- LED  
- GPIO  
- Motor Control  
- Buzzer  
- I2C-CLCD  
- GPS 관련 (UTM 좌표 변환)

### 1.2 Arduino
- Arduino로 복원  
- GPIO  
- Motor Control  
- Buzzer  
- I2C-CLCD  

---

## 2. Raspberry PI 4 (4GB) — 💰 예상가격 약 822,800원  
🔗 [Micro SD Card 32GB 포함](https://www.11st.co.kr/products/6645724808) — 💰 예상가격 약 5,660원  

<img width="320" height="164" alt="028" src="https://github.com/user-attachments/assets/525b3b8e-80f4-4582-9d5a-e94d216ff4e6" />

https://smartstore.naver.com/daesinman/products/11361679545

<img width="320" height="240" alt="026" src="https://github.com/user-attachments/assets/7054186a-9665-42b6-b8c1-74344475ecbd" />

### 2.1 리눅스 기본 명령어
### 2.2 파이썬 프로그래밍
### 2.3 GPIO 제어 : 센서, 모터, 통신
### 2.4 네트워크 및 웹서버 구축, 카메라 제어, Pan/Tilt

## 3. 도서
   * 도서 : 2026.1.8 임베디드 지능로봇시작 예정
   * 이것이 C언어다 : 25,000(책 확인중)
   * 라즈베리파이 5와 40개의 작품들 : 22,000 (책 확인중)
   * 라즈베리파이 4로 변경하는것이 좋지 않을까?
   * 사물인터넷을 위한 리눅스 프로그래밍 with 라즈베리파이 : 38000(책 확인중)
   * STM32 따라하기 : 22,000(책 확인중)
   * 마이크로컨트롤러 프로그래밍 : 33,000
   * 구매 리스트 삭제 : 파이썬 관련 도서로  변경하는것이 좋을것 같음


